...
func NewLogStreamer(client *kubernetes.Clientset, namespace string, out io.Writer, logger *logrus.Logger) *LogStreamer {
	return &LogStreamer{
		client:    client,
		namespace: namespace,
		out:       out,
...
func (ls *LogStreamer) StartLogStream(ctx context.Context, podName, containerName string, isExistingPod bool) error {
	...
	// For existing pods show only the last 10 lines
	if isExistingPod {
		tailLines := int64(10)
	...
	if err != nil {
		ls.logger.WithField("stream", streamKey).WithError(err).Error("Failed to create log stream")
		return err
	}
	defer stream.Close()

	if err := ls.copyWithLogrus(ctx, stream, podName, containerName); err != nil && err != context.Canceled {
		ls.logger.WithFields(logrus.Fields{
			"stream":    streamKey,
			"pod":       podName,
			"container": containerName,
...
func (ls *LogStreamer) waitContainerReady(ctx context.Context, podName, container string) {
	...
	for {
		...
		case <-t.C:
			...
			if err != nil {
				ls.logger.WithField("stream", streamKey).WithError(err).Warning("Failed to get pod while waiting for container")
				return
			}
...
func (ls *LogStreamer) copyWithLogrus(ctx context.Context, src io.Reader, podName, containerName string) error {
